...
SM_KERNEL_VERSION := $(filter 4.8.4 4.8.5 4.8.6 4.9.1 4.9.2 4.9.3 4.9.4 4.10.x%,$(SM_KERNEL))
...
ifeq (true,$(USE_SM_TOOLCHAIN))
export LD_LIBRARY_PATH := $(ANDROID_BUILD_TOP)/prebuilts/gcc/$(HOST_PREBUILT_TAG)/aarch64/aarch64-linux-android-$(TARGET_FUSION_ROM)/arch-arm64/usr/lib
export LIBRARY_PATH := $(ANDROID_BUILD_TOP)/prebuilts/gcc/$(HOST_PREBUILT_TAG)/aarch64/aarch64-linux-android-$(TARGET_FUSION_ROM)/arch-arm64/usr/lib
endif

# Path to toolchain
SM_AND_PATH := prebuilts/gcc/$(HOST_PREBUILT_TAG)/aarch64/aarch64-linux-android-$(TARGET_FUSION_ROM)
...
ifeq (true,$(FUSION_KRAIT))
 OPT4 := krait|
endif

ifeq (true,$(FUSION_ENABLE_GCCONLY))
 OPT5 := gcconly|
endif
...
ifeq (true,$(FUSION_FFAST_MATH))
 OPT7 := ffast_math|
endif

ifeq (true,$(FUSION_PIPE))
 OPT8 := pipe
endif
